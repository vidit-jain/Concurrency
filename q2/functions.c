...
void* goalThread(void* args) {
    ...
    while (curr_goal_chance < goal_count) {
        ...
        if (scored(goals[curr_goal_chance]->chance)) {
            ...
            printf("t = %d: Team %c scored their %d%s goal\n", seconds_elapsed, zone_symbol[trigger], goals_scored[trigger], suffix(goals_scored[trigger]));
        ...
        // If the team doesn't score
            ...
            pthread_mutex_lock(&print_lock);
            pthread_mutex_lock(&goal_lock);
            printf("t = %d: Team %c missed the chance to score their %d%s goal\n", seconds_elapsed, zone_symbol[x], goals_scored[x] + 1, suffix(goals_scored[x] + 1));
            pthread_mutex_unlock(&goal_lock);
            pthread_mutex_unlock(&print_lock);
        }
        ...
        for (int i = 0; i < num_groups; i++) {
            for (int j = 0; j < groups[i]->spectator_count; j++) {
                ...
                if (s->current_seating != -1) {
                    if (trigger == 1 - s->side && goals_scored[1 - s->side] >= s->max_pain) {
                        s->wakeup_reason = 1;
                        unlocked = 1;
...
void *spectatorThread(void *arg) {
    ...
    if (sem_timedwait(&available_seats[pref], &ts) == 0){
        ...
        if (s->current_seating == -1) {
            ...
            pthread_mutex_lock(&print_lock);
            printf(MAG"t = %d: %s has got a seat in zone %c\n"WHT, seconds_elapsed, s->name, zone_symbol[s->current_seating]);
            s->exit_time = seconds_elapsed + spectating_time;
            pthread_mutex_unlock(&print_lock);

            pthread_mutex_lock(&goal_lock);
            // Checking if he should watch the match at all if the opposing team enraged him enough
            if (s->side != 2 && goals_scored[1 - s->side] >= s->max_pain) {
                s->wakeup_reason = 1; // Act like he already woke up if he is enraged
                pthread_mutex_unlock(&s->lock);
            }
            else {
                pthread_mutex_unlock(&s->lock);
    ...
    if (s->queues_left == s->allowed_zones) {
        ...
        // Thread was triggered cause team scored too many goals
            ...
            printf(GRN"t = %d: %s is leaving due to bad performance of his team\n"WHT, seconds_elapsed, s->name);
        }
        // Guy got other plans and left
        else if (s->wakeup_reason == 2) {
            printf(GRN"t = %d: %s watched the match for %d seconds and is leaving\n"WHT, seconds_elapsed, s->name, spectating_time);
        }
        // Waiting at the gate
    ...
    for (int i = 0; i < g->spectator_count; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (s->side == j || s->side == 2 || (s->side == 0 && j == 2)) {
                ...
                w->spec = s;
                w->choice = j;
                pthread_create(&spectator_threads[i][j], NULL, spectatorThread, (void*) w);
            }
        }
    }
    for (int i = 0; i < g->spectator_count; i++) {
