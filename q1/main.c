...
int main() {
	...
	for (int i = 0; i < course_count; i++) {
		...
		courses[i] =
			createCourse(name, interest, course_max_slot, lab_ids, allowed_labs, i);
	}
    ...
    for (int i = 0; i < student_count; i++) {
        ...
        pthread_create(&currThread, NULL, studentThread,
                       (void *)students[i]);
        student_threads[i] = currThread;
    }
    // Lab threads before course as it's a dependency of course threads
    pthread_t lab_threads[lab_count];
    for (int i = 0; i < lab_count; i++) {
        pthread_t currThread;
        pthread_create(&currThread, NULL, labThread,
                       (void *)iiit_labs[i]);
    ...
    for (int i = 0; i < course_count; i++) {
        pthread_t currThread;
        pthread_create(&currThread, NULL, courseThread,
                       (void *)courses[i]);
